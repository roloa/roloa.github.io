...
export class MaterialBalance {
    ...
    get_fishing_result(){
        ...
        if( item_type < 30 ){
            let fishing_result = new ResourceItem( this.game );
            fishing_result.saving_data.item_name = '流木';
            fishing_result.set_image( 'tree_ryuuboku' );
            fishing_result.add_material( 'wood', Math.floor( 20 + Math.random() * 10 ) );
            ...
            fishing_result.add_material( 'stone', Math.floor( 10 + Math.random() * 10 ));
            return fishing_result;
        }
        // 魚
    ...
    get_drifting_item(){
        ...
        if( item_type < 30 ){
            ...
            drifting_item.add_material( 'wood', Math.floor( 20 + Math.random() * 10 ));
        } else if( item_type < 60 ){
            drifting_item = new ResourceItem( this.game );
            drifting_item.saving_data.item_name = '古着';
            drifting_item.set_image( 'alohashirt_gray' );
            drifting_item.add_material( 'cloth', Math.floor( 10 + Math.random() * 10 ));
        } else if( item_type < 90 ){
            drifting_item = new ResourceItem( this.game );
            drifting_item.saving_data.item_name = '空き缶';
            drifting_item.set_image( 'gomi_can' );
            drifting_item.add_material( 'iron', Math.floor( 10 + Math.random() * 10 ));
