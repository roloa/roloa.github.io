...
export class HudStatus {
    ...
    draw_guage( canvas, dan , fill_rate){
        ...
        // 塗り部分
        canvas.beginPath(  );
        canvas.moveTo( 0                                   ,         dan * HudStatus.STATUS_HEIGHT );
        canvas.lineTo( HudStatus.GUAGE_LENGTH * fill_rate  ,         dan * HudStatus.STATUS_HEIGHT - HudStatus.GUAGE_TILT * fill_rate);
        canvas.lineTo( HudStatus.GUAGE_LENGTH * fill_rate  ,    20 + dan * HudStatus.STATUS_HEIGHT - HudStatus.GUAGE_TILT * fill_rate);
        canvas.lineTo( 0                                   ,    20 + dan * HudStatus.STATUS_HEIGHT );
        canvas.closePath();
        canvas.fill();
        ...
        canvas.beginPath(  );
        canvas.moveTo( 0                     ,         dan * HudStatus.STATUS_HEIGHT );
        canvas.lineTo( HudStatus.GUAGE_LENGTH,         dan * HudStatus.STATUS_HEIGHT - HudStatus.GUAGE_TILT );
        canvas.lineTo( HudStatus.GUAGE_LENGTH,    20 + dan * HudStatus.STATUS_HEIGHT - HudStatus.GUAGE_TILT );
        canvas.lineTo( 0                     ,    20 + dan * HudStatus.STATUS_HEIGHT );
        canvas.closePath();
        canvas.stroke();
    }
    on_draw( canvas ){
        ...
        canvas.drawImage( this.icon_hp, HudStatus.ICON_X, HudStatus.ICON_Y + 0 * HudStatus.STATUS_HEIGHT, 24, 24 );
        ...
        canvas.drawImage( this.icon_thirst, HudStatus.ICON_X, HudStatus.ICON_Y + 4 * HudStatus.STATUS_HEIGHT, 24, 24 );
