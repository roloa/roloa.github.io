export class BallBase {
    ...
    crash_bound( tgt ){
        ...
        vx /= vec_len
        vy /= vec_len
        this.x -= vx * overlap_distance * 0.51
        this.y -= vy * overlap_distance * 0.51
        tgt.x += vx * overlap_distance * 0.51
        tgt.y += vy * overlap_distance * 0.51

        // 運動エネルギーの和
        let my_speed_p2 = Math.sqrt(this.dx * this.dx + this.dy * this.dy)
        let tgt_speed_p2 = Math.sqrt(tgt.dx * tgt.dx + tgt.dy * tgt.dy)
        let kinetic_energy_sum = (my_speed_p2 * this.mass + my_speed_p2 * tgt.mass) * this.bounce * tgt.bounce
        ...
        this.dx = -vx * kinetic_energy_sum  / this.mass * 0.7
        this.dy = -vy * kinetic_energy_sum  / this.mass * 0.7
        tgt.dx = vx * kinetic_energy_sum  / tgt.mass * 0.7
        tgt.dy = vy * kinetic_energy_sum  / tgt.mass * 0.7
    }
    ...
    on_update_ball_phisical( game ){
        ...
        if( game.field.height - this.radius < this.y ) {
            ...
            if(this.dy <= 1){
                ...
                this.dy = this.dy * -0.95 * this.bounce
            }
            this.drot += this.dx * 0.033
            this.drot *= 0.5
            this.dx *= 0.95
        }
        if( game.field.width - this.radius < this.x ) {
            this.x = game.field.width - this.radius
            this.dx = this.dx * -0.7 * this.bounce
        }
        ...
        if( this.y < this.radius ) {
            this.y =  this.radius
            this.dy = this.dy * -0.7 * this.bounce
        }
